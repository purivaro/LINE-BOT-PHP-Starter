...
foreach ($events as $event) {
	...
	if($eventType == 'join'){
		...
		$ref_group = $database->getReference('japan/line/contact/group');
	...
	if($eventType == 'leave'){
		...
		foreach($data as $key => $value){
			if($GroupId==$value['GroupId']){
				$database->getReference('japan/line/contact/group/'.$key)->remove();
			}
		}
	...
	if($eventType == 'message'){
		...
		$database->getReference("japan/line/contact/user/{$row_key}/ChatLastText")->set($text);
		$database->getReference("japan/line/contact/user/{$row_key}/ChatLastTimestamp")->set(-1*$timestamp);
		$database->getReference("japan/line/contact/user/{$row_key}/Unread")->set($Unread+1);
